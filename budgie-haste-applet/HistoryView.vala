...
public class HasteApplet.HistoryView : Gtk.Box
{
    ...
    public HistoryView(GLib.Settings settings, Gtk.Clipboard clipboard)
    {
        ...
        Gtk.Label header_label = new Gtk.Label("<span font=\"11\">%s</span>".printf(_("Recent Hastes")));
        ...
        clear_all_button = new Gtk.Button.with_label(_("Clear all Hastes"));
        clear_all_button.get_child().margin = 5;
        clear_all_button.get_child().set_margin_start(0);
        ...
        Gtk.Label placeholder_label = new Gtk.Label("<big>%s</big>".printf(_("Your hastes will show up here")));
    ...
    public void update_child_count()
    {
        ...
        clear_all_button.set_sensitive(!(len == 0));
    }
    ...
    public async void update_history(int n, bool startup)
    {
        ...
        if (history_listbox.get_children().length() != 0) {
            ...
            separator.set_can_focus(separator_item.selectable = false);
            separator_item.set_can_focus(separator_item.activatable = false);
